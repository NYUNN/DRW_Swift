...
class RecorderViewController: UIViewController {
    ...
    @IBAction func record(_ sender: UIButton) {
        ...
        // when recording button is clicked firstly
        if recorder == nil {
            ...
            // 2. stop background sound
            audioPlayer.stop()
            
            // 3. start record!! after 5 seconds (must required)
        ...
        if recorder != nil && recorder.isRecording { //  when clicked while recording
            
            // 1. stop and store recording
            recorder?.stop()
            player?.stop()
            audioPlayer.stop()
        ...
        } else { //  when button is clicked during paused
            
            // re-record with count playing
            count.play()
            let countTimer : Timer = Timer.scheduledTimer(timeInterval:5, target: self, selector: #selector(self.recordVoice), userInfo: nil, repeats: false)
            return
        }
